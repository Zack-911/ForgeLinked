...
exports.default = new forgescript_1.NativeFunction({
    ...
    async execute(ctx, [guildId, speed, pitch, rate]) {
        try {
            ...
            await player.filterManager.toggleVaporwave(speed, pitch, rate);
            return this.successJSON({
                success: true,
                active: player.filterManager.filters.vaporwave,
                filters: player.filterManager.filters,
                data: player.filterManager.data,
            });
        }
        catch (err) {
            return this.customError(`Failed to toggle vaporwave: ${err instanceof Error ? err.message : String(err)}`);
        }
    },
});
